...
std::string encryptData(const char* data, size_t dataSize) {
    // Инициализация ключа и вектора инициализации
    // Вам нужно заменить эти значения на свои собственные
    unsigned char key[AES_BLOCK_SIZE] = "7372";
    unsigned char iv[AES_BLOCK_SIZE] = "jsjeu";

    AES_KEY aesKey;
    AES_set_encrypt_key(key, 128, &aesKey);

    // Дополнение данных до размера, кратного блоку
    int padding = AES_BLOCK_SIZE - (dataSize % AES_BLOCK_SIZE);
    size_t paddedDataSize = dataSize + padding;
    std::string paddedData(paddedDataSize, '\0');
    memcpy(&paddedData[0], data, dataSize);
    for (int i = dataSize; i < paddedDataSize; ++i) {
        paddedData[i] = padding;
    }

    // Выделение буфера для зашифрованных данных
    std::string encryptedData(paddedDataSize, '\0');

    // Шифрование данных
    AES_cbc_encrypt(reinterpret_cast<const unsigned char*>(paddedData.c_str()), 
                    reinterpret_cast<unsigned char*>(&encryptedData[0]), 
                    paddedDataSize, &aesKey, iv, AES_ENCRYPT);
...
void *imgui_go(void *) {
    ...
    if(Auth()){
		...
		auto il2cppHandle = dlopen(LIB, RTLD_LAZY);
    	il2cpp_string_new = (MonoString *(*)(const char *)) dlsym(il2cppHandle, "il2cpp_string_new");
    }
    return nullptr;
}
