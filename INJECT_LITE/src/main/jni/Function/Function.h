...
bool Offline(void *player) {
    void *PlayerInfos = *(void**) ((uint64_t) player + 0x258);
    if (PlayerInfos) {
...
MonoString *GetPlayerNick(void *player){
    void *PlayerNames = *(void**) ((uint64_t) player + 0x258);
    if(PlayerNames){
...
int GetCurrentWeaponID(void* player) {
    return *(int*)((uintptr_t)player + 0x1F8);
}
...
void PlayerController_Update(void* myplayer){
    if (myplayer != NULL){
        ...
        void *m_Rigidbody = *(void **)((uint64_t)myplayer + 0x2A4);
		...
		if(NearWeapon != 0){
			itemid = *(int*) ((uint64_t)NearWeapon + 0x60);
		}
        ...
        if (MyPlayerData.FastHit) {
			if(NearWeapon != 0){
				void *HandAttack = *(void **)((uint64_t)NearWeapon + 0x5C);
				OnTriggerEnter1(HandAttack);
			}
		}
		void *vThirdPersonCamera = *(void**)((long)myplayer + 0x224);
...
const char *DumpType = "UnityEngine.GameObject, UnityEngine";
void saveLog(const std::string& message, std::string fileName) {
    std::ofstream logFile;
    std::string filePath = "/sdcard/Android/data/com.herogame.gplay.survival.lite/files/" + fileName;
    ...
    if (logFile.is_open()) {
        logFile << logMessage;
        logFile.close();
        LOGI("Лог сохранен успешно.");
    } else {
        LOGE("Ошибка при сохранении лога.");
    }
}
MonoString *(*il2cpp_string_new)(const char *cstr);
MonoString *(*get_tag)(void *object);
MonoArray <void **> *(*FindObjectsOfType)(void *);
MonoString *(*get_name)(void *object);
void *(*GetType)(MonoString *type);
bool Testbool;
void DumpObjects() {
     MonoArray<void **> *objects = NULL;
     void *get_type = NULL;
     
     get_type = GetType(il2cpp_string_new(DumpType));

     if (get_type != NULL) {
         objects = FindObjectsOfType(get_type); 
     }
     
     LOGE("OBJECTS = %s", objects);
     if (objects != NULL && Testbool) {
         std::string r = "*******************  Mono Array ";
         std::string a = to_string(objects->getLength());
         std::string u = " items,crash possible *****************";
         saveLog(r + a + u, "dump_objects_all_scene.cs");
         
         for (int i = 0; i < objects->getLength(); i++) {
             void *obj = objects->getPointer()[i];       
             if (obj != NULL && objects->getLength() > 1) {              
                 std::string s = get_tag(obj)->toChars();
                 std::string h = get_name(obj)->toChars();
                 ...
                 saveLog(AllObjects, "dump_objects_all_scene.cs");   
             }
         }
     }
}
...
int layer;
MonoString *targetStateName;
int stateHash;
bool isCross;
void (*_baobap)(void *player, int a, MonoString *b, int c, bool d);
void baobap(void *player, int a, MonoString *b, int c, bool d){
	_baobap(player, a, b, c, d);
	layer = a;
	targetStateName = b;
	stateHash = c;
	isCross = d;
